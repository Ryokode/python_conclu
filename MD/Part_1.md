...
#结果为：([29,23,53,55],[10,34,44])
```
- 如果函数没有返回值（函数执行完毕后，不需要给调用处提供数据），return可以不写
- 函数的返回值如果是1个，直接返回类型
- 函数返回多个值是，结果为元组
...
![image.png](https://raw.githubusercontent.com/Ryokode/PicGo/main/20260822130235567.png)
#### 个数可变的位置参数
- 定义函数时，可能无法实现确定传递位置参数的个数时，使用可变位置参数
- 使用\*定义个数可变的位置形参
- 结果为一个元组
- 只能有一个个数可变的位置参数

![image.png](https://raw.githubusercontent.com/Ryokode/PicGo/main/20260822130636711.png)
#### 个数可变的关键字形参
-  定义函数时，可能无法实现确定传递关键字实参的个数时，使用可变的关键字形参
- 使用\*\*定义个数可变的关键字形参
- 结果为一个字典
- 只能有一个个数可变的关键字形参

![image.png](https://raw.githubusercontent.com/Ryokode/PicGo/main/20260822130841311.png)
...
![image.png](https://raw.githubusercontent.com/Ryokode/PicGo/main/20260822141907688.png)
#### 多个except结构
...
class Student:
	def __init__(self,age):
		slef.__age = age;    #年龄不希望在类的外部被使用，所以加了两个__
	def get_age(self):
		return self.__age;
...
print(dir(o));
print(dir(p));
print(p);#直接输出时默认调用__str__()方法
 ```
#### 多态
- 简单地说，多态就是”具有多种形态“，它指的是：即便不知道一个变量所引用的对象到底是什么类型，仍然可以通过这个变量调用方法，在运行过程中根据变量所引用对象的类型，动态决定调用哪个对象中的方法
![image.png](https://raw.githubusercontent.com/Ryokode/PicGo/main/20260822180215367.png)
```python
class Animal(object):
	def eat(self):
		print('动物要吃东西');
...
#只要类中有eat()方法就可以输出
```
![image.png](https://raw.githubusercontent.com/Ryokode/PicGo/main/20260822180651407.png)
##### 静态语言与动态语言
- 静态语言实现多态的三个必要条件
	- 继承
	- 方法重写
